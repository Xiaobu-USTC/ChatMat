...
from ChatMat.tools.Dataset_search.prompt import DF_PROMPT
    ...
    def _call(
        ...
        for i in range(max_iteration + 1):
            ...
            if not llm_output.strip():
                ...
                if isinstance(llm_output, dict):
                    llm_output = llm_output.get('text', '')
                # print(f"LLM Output after empty check: {llm_output}")
            ...
            if re.search(r'Final Answer: (success|.* above|.* success|.* succeed|.* DataFrames?).?$', llm_output):
                ...
                # print(f"LLM Output after Final Answer check: {llm_output}")
            ...
            # if num_tokens > 3400:
                # Implement token limit handling here
    ...
    def from_filepath(
        ...
        **kwargs
        ...
        template = PromptTemplate(
            template=prompt,
            input_variables=['df_index', 'information', 'df_head', 'input', 'agent_scratchpad']  # Changed 'question' to 'input'
        )
        llm_chain = LLMChain(llm=llm, prompt=template)
        df = cls._get_df(file_path)
        # print(f"********{df}")
        # print(df.loc[df['name']=='Al176Si24', 'energy(eV)'].to_markdown())
        encode_function = llm.get_num_tokens

        # Extract 'verbose' from kwargs to prevent duplication
        ...
        return cls(
            ...
            verbose=verbose,
            **kwargs  # Remaining kwargs without 'verbose'
        )
